...
add_multiple(Node,N)->
    ...
    [After|Trace]=add_multiple(Node,N,[]),
    Diff=After-Before,
    io:format("Total time to add ~w data in microseconds: ~w~n", [N, Diff]),
    Trace.

add_multiple(Node,N,Trace)->
    case N of
        0->
            [erlang:system_time()|Trace];
        _->
            Rnd=key:generate(),
            Value=key:generate(),
            Q = make_ref(),
            Node ! {add, Rnd, Value, Q, self()},
            receive 
                {Q, ok} ->
                    add_multiple(Node,N-1,[Rnd|Trace])
                end
            end.
